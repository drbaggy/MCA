(function () {
  ...
  function load_data( id ) {
    ...
    var plotdata, counter, graph = {}, current_gene;
    Plotly.d3.json( 'data/' + (_.qs(id).dataset.directory) + '/' + FILE_NAME, function(err, plotdata) {
      var config = plotdata.config;
      var genes  = plotdata.genes;
      var data   = plotdata.data;
// Create the hover templates...
      counter = 0;
      graph.hover_template      = config.columns.map( a => a.name+': %{customdata['+(counter++)+']}' ).join('<br>');
      graph.hover_template_expr = graph.hover_template+'<br>Expr.: %{text}<extra></extra>';
      graph.hover_template     += '<extra></extra>';
      graph.genes_length        = genes.length;
// Process the data to get the unmapped customdata and gene colours (by stage/day);
      graph.default_text = data.CUSTOMDATA.map( a => '-' );
      graph.visible      = data.CUSTOMDATA.map( a => config.marker_size );
      graph.customdata   = data.CUSTOMDATA.map( function(a) { counter = 0; return a.map( b => config.columns[counter++].colours[b][0] ); } );
      graph.colours = { gene: graph.customdata.map( a => config.expression_default ) };
      counter = 0;
      config.columns.forEach( function(a) {
        graph.colours[ a.name.toLowerCase() ] = data.CUSTOMDATA.map( b => config.columns[counter].colours[b[counter]][1] );
        counter++;
      });
      ...
      function exp_colour(a,mx) {
        ...
        var o = a/mx - i/8;
        var p = 1 - o;
        return 'rgb('+ ( config.expression_colours[i][0]*p+config.expression_colours[i+1][0]*o ) +','+
                       ( config.expression_colours[i][1]*p+config.expression_colours[i+1][1]*o ) +','+
                       ( config.expression_colours[i][2]*p+config.expression_colours[i+1][2]*o ) +')';
      }

// Draw graph
      var def = config.columns[0].name.toLowerCase();
      var points = {
      ...
      Plotly.newPlot('pf-new-pca-graph', points.PC, { autosize: true, margin: MARGIN,
        scene: { xaxis: { range: config.ranges.PC[0], title: 'PC 1' },
                 yaxis: { range: config.ranges.PC[1], title: 'PC 2' },
                 zaxis: { range: config.ranges.PC[2], title: 'PC 3' }, }}, OPTIONS );
      Plotly.newPlot('pf-new-umap-graph', points.UMAP, { autosize: true, margin: MARGIN,
        scene: { xaxis: { range: config.ranges.UMAP[0], title: 'UMAP 1' },
                 yaxis: { range: config.ranges.UMAP[1], title: 'UMAP 2' },
                 zaxis: { range: config.ranges.UMAP[2], title: 'UMAP 3' }, }}, OPTIONS );
...
// Now add actions on change filters....
      function changeFilter( n ) { n.onchange = function(e) {
        ...
        graph.visible = graph.customdata.map( a => config.marker_size * filter_set[a[0]] * filter_set[a[1]] );
...
// Now add actions on change colour set
      ...
      function changeGene( n ) { n.onkeyup = function(e) {
        ...
        if( new_gene === '' ) {
          ...
          graph.colours.gene = plotdata.data.CUSTOMDATA.map( a => config.default_expression ); // reset colours
        ...
        Plotly.d3.json( 'data/' + (_.qs(id).dataset.directory) + '/expression/'+new_gene+'.json', function(err,expdata) {
          ...
          if( max_exp == 0 ) {
            graph.colours.gene = expdata.data.map( a => config.default_expression );
            _.m('.gradient span', a => a.innerText = '-');
          } else {
